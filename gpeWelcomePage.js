...
-180deg, rgba(255, 255, 255, 0.25) 0%, rgba(255, 255, 255, 0) 100%) rgb(0, 91, 240);
    box-shadow: rgb(0 0 0 / 4%) 0px 1px 2px 0px;
    height: 40px;
    padding: 2px 24px 0px;
    font-size: 14px;
}
.getstarted_button:hover:not(:disabled) {
    background: linear-gradient(to right, rgba(0, 0, 0, 0.08) 0%, rgba(0, 0, 0, 0.08) 100%), linear-gradient(
-180deg, rgba(255, 255, 255, 0.25) 0%, rgba(255, 255, 255, 0) 100%) rgb(0, 91, 240);
}
...
* 
MGR
24,2,31,3,15,26,32,38,48,49,14,13,12,19,41,47,46,28,30

HRD
24,27,2,16,29,31,36,3,15,17,22,25,26,32,38,48,14,13,12,19,44,42,46,28

INS
14,12,41,47,44,42,43,46,28,30

ADM
27,16,29,31,36,50,12,44,43,46,28
...
*/			
		...
		.then(data => {
//			console.log(" hide loader ]*"+ widgetIDArg.id +"*******************************************************************************************************************************");
...
function buildQuickLinksCard(accessArrArg, cultureArg){
	...
	/* BTN START */
	var qlArrTmp = multiSort(getAccessDetails(accessURLs), {module: 'asc', quicklinkPrio: 'asc'});
	var qlArr = {};
...
/*
	return fetch(sessionStorage.csCloud +"perf-conversations-api/v1/conversations/participants", {
		method: 'GET',
		mode: 'cors',
		cache: 'no-cache',
	...
	.then(partResp => {
		...
		var keys = $.map(partResp.directReports, function(v, i){
			return v["id"];
		});
...
//				(".................. DIRECT REPORTS FOUND ..................");
			var async_request = [];
			var goalResponses = [];
			var emplData = partResp.directReports;
			return fetch("/services/api/x/users/v2/employees?ids="+keys.toString(), {
				method: 'GET',
				mode: 'cors',
				cache: 'no-cache',
				credentials: 'same-origin',
...
//			console.log("HERE I AM!");
	...
	//				checkinStr += "<a href='/ui/perf-check-ins/Check-Ins/view/"+ checkinObjects[checkinItem]["id"] +"/meeting/"+ checkinObjects[checkinItem]["meetingsSummary"]["nextMeetingId"] +"'>";
				...
				var lastModMonth = new Date(checkinObjects[checkinItem]["lastModifiedDate"]).toLocaleDateString('en-GB', { month : 'short'}).split(' ');
				var lastModDay = new Date(checkinObjects[checkinItem]["lastModifiedDate"]).toLocaleDateString('en-GB', { day : 'numeric' }).split(' ');
	//				var lastMod = new Date(checkinObjects[checkinItem]["lastModifiedDate"]).toLocaleDateString('en-GB', { day : 'numeric',  month : 'short'}).split(' ').join('-');
				...
				if(checkinObjects[checkinItem]["meetingsSummary"]["isNextMeetingCreated"]){
					var nextMeetingMonth = new Date(checkinObjects[checkinItem]["meetingsSummary"]["nextMeetingDate"]).toLocaleDateString('en-GB', { month : 'short'}).split(' ');
					var nextMeetingDay = new Date(checkinObjects[checkinItem]["meetingsSummary"]["nextMeetingDate"]).toLocaleDateString('en-GB', { day : 'numeric' }).split(' ');
	...
	tmpFeedContentDiv.setAttribute("id", contentDivClassArg);
		...
		$("tr[id^='ctl00_ContentPlaceHolder1_widgetLayout_rptWidgets_'][id*='widgetContainer_ctl00_rChatter_']").each(function(index, value){
			...
			if(isNaN(theDate.getTime())){
				...
				var actionDay = theDate.toLocaleDateString('en-GB', { day : 'numeric'}).split(' ');
				var actionMonth = theDate.toLocaleDateString('en-GB', { month : 'short'}).split(' ');
...
/*
            for(var labelIndex in labels){
                chBgColor.push(palette.filter(function(item) {
					return item.displayName === labels[labelIndex];
                })
                .map(function(item) {
					return item.color
                }))
...
//				backgroundColor: chBgColor,
			...
			for(var i in reportCols){
				...
				chartData.datasets.push({
					label: reportCols[i],
					backgroundColor: reportData.chartPalette.filter(function(item) {return item.displayName === reportCols[i]}).map(function(item) {return item.color}),
...
//		console.log("chartTitleArg: "+ chartTitleArg);
//		console.log("cs_report_+reportID: cs_report_"+ reportIDArg);
/*
		let el = document.querySelector("#cs_report_"+reportIDArg+" .card-body");
		el.innerHTML = "<a data-bs-toggle='modal' data-bs-target='#modalTable_"+reportIDArg+"' >"+ el.innerHTML+"</a>";
*/
...
//			console.log(response);
			//return [tempcol, , reportIDArg, reportData.title];
...
//		return [generateColumns(reportTBLColumns), generateReportData(reportTBLData, reportTBLColumns), reportIDArg, reportData.title];
	})
	.then(reponse => {
		//console.log(reponse);
		return reponse;
...
var getReportData = async (reports, demoRoleArg) => {
	const requests = reports.map((reportID) => {
		...
		// arg 3 = 
		// arg 4 = Where to place the chart
		//function createDashboard(reportIDArg, chartTitleArg, chartDivTitleArg, chartDivTargetArg){

		var tmpCanvas = document.createElement("canvas");
		tmpCanvas.setAttribute("id", "report"+reportID);
		tmpCanvas.setAttribute("style", "height: 250px, width: 100%");

//		buildCard_NEW("", "#", cs_DashboardDetailsArray.reports[reportID].width, "cs_report_"+reportID, "cs_report", demoRoleArg+"-right", "reportContents", tmpCanvas);
	...
	.then(reportResponseData => {
		...
		tempReport = reportResponseData;

		// for each report, create modal data
		tempReport.forEach(function(reportResponse){
...
$(function () {
	...
	.then(() => {
		...
		console.log("If we're evil or divine");
		console.log("We're the last in line");
	})
	.catch(error => {
		console.error(error);
	});
